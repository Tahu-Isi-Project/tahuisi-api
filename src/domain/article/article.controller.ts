...
article.patch("/:slug", userAuthMiddleware, adminCheckMiddleware, validateSlugParam, validateArticleUpdateBody, async (c) => {
  const { slug } = c.req.valid("param");
...
article.delete("/:slug", userAuthMiddleware, adminCheckMiddleware, validateSlugParam, async (c) => {
  const { slug } = c.req.valid("param");
...
// for development
article.delete("/", userAuthMiddleware, adminCheckMiddleware, async (c) => {
